Jonathan D.A. Jewell <j.d.a.jewell@open.ac.uk> :toc:
...
heuristic `+select_backend+` / `+estimate_cost+` / `+fits_on_device+`
API.
...
write-once-run-on-any-backend kernels and array wrapping. These overlap
conceptually with AcceleratorGate’s selection idea and are more mature
for kernel portability.
...
accelerators installed — backends report `+available+` `+⇒+` `+false+`
and selection falls back to `+JuliaBackend+`.)
